...
| ボード概念の導入、グループ作成・招待・共有（フェーズ5基本機能） | 担当者割当・リーダー権限・提案フロー・通知（5.5.1 将来検討事項） |
| 日本語UI | 多言語対応 |
| ライトテーマのみ | ダークモード |
...
**補足：**
- フェーズ5でグループ・ボード機能を導入する際もこの3層構成は変わらず、データベース内のテーブル構成のみ変更される（9.2.2 ER図 参照）
- 将来的に通知機能や提案フローを追加する際は、メール送信サービスや非同期ジョブ基盤などの外部要素が加わる可能性がある（5.5.1 将来検討事項 参照）
...
| 2026-05-08 | 1.14 | 第8章を「使用技術・システム構成」に改名し、8.4 システム構成図 を追加（フェーズ1〜2のブラウザ単独構成／フェーズ3以降のWeb3層構成）。 |
| 2026-05-08 | 1.15 | 対象範囲（1.4 スコープ）を追加。スコープ内／スコープ外を機能・環境・運用・データの4軸で対比表として明記。 |
